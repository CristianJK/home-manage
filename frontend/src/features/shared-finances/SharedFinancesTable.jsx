  ...
  return (
    <div className="bg-surface border border-outline rounded-xl shadow-sm overflow-hidden">
      <table className="w-full text-left border-collapse">
        ...
        <tbody className="divide-y divide-outline">
          {displayed.map((expense) => {
            ...
            return (
              <tr key={expense.id} className="hover:bg-surface-variant transition-colors group">
                <td className="p-4 text-sm text-text-secondary">{formatDate(expense.due_date)}</td>
                <td className="p-4">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      {expense.comment && <p className="text-xs text-text-secondary">{expense.comment}</p>}
                    </div>
                  </div>
                </td>
                <td className="p-4">
                ...
                <td className="p-4 text-right text-sm font-bold">${formatCurrency(expense.amount)}</td>
                <td className="p-4 text-center">
                  <span className={`text-xs font-medium px-2 py-1 rounded-full ${expense.is_paid ? "bg-emerald-100 text-emerald-700" : "bg-amber-100 text-amber-700"}`}>
      ...
      <div className="p-4 bg-surface border-t border-outline flex justify-center">
        <button onClick={onViewAll} className="text-primary text-xs font-medium flex items-center gap-1 hover:underline">
