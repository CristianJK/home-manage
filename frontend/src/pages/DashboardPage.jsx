...
export default function DashboardPage() {
  ...
  const urgentPayment = useMemo(
    () =>
      expenses.filter((e) => !e.is_paid).sort((a, b) => new Date(a.due_date) - new Date(b.due_date))[0] || null,
    [expenses]
  );

  const upcomingPayments = useMemo(
    () =>
      expenses.filter((e) => !e.is_paid).sort((a, b) => new Date(a.due_date) - new Date(b.due_date)).slice(0, 4),
    [expenses]
  );
        ...
        <div className="grid grid-cols-1 md:grid-cols-12 gap-4">
              ...
              <div className="flex flex-col lg:flex-row items-center justify-around gap-4 py-4">
                <div className="relative w-48 h-48 rounded-full border-[16px] border-primary flex items-center justify-center">
                  <div className="text-center">
                    <span className="text-xl font-bold block">${formatCurrency(totalMonth)}</span>
                    <span className="text-xs text-text-secondary">Total Mes</span>
                  </div>
                </div>
          ...
          <div className="md:col-span-4 flex flex-col gap-4">
            {urgentPayment && (() => {
              ...
              return (
                ...
                >
                  <span className={`material-symbols-outlined ${urgency === "Vencido" ? "text-error" : "text-amber-600"}`}>
                    {urgency === "Vencido" ? "error" : "warning"}
                  </span>
                  <div className="flex flex-col gap-1">
                    <span className="text-base font-bold">
                      {urgency === "Vencido" ? "Pago Vencido" : "Pago Urgente"}
                    </span>
                    <p className="text-sm">
                      {conceptConfig[urgentPayment.concept]?.label || urgentPayment.concept}{" "}
                      {urgency === "Hoy" ? "vence hoy" : `vence en ${urgency}`}.
                    </p>
            ...
            <div className="bg-surface border border-outline rounded-xl p-4 shadow-sm flex flex-col gap-3 flex-1">
              ...
              {upcomingPayments.length === 0 ? (
                <p className="text-sm text-text-secondary py-2">No hay pagos pendientes</p>
              ) : (
                <div className="flex flex-col gap-1">
                  {upcomingPayments.map((exp) => (
                    <div key={exp.id} className="flex justify-between items-center p-2 rounded-lg hover:bg-surface-variant transition-colors">
          ...
          <div className="md:col-span-6 bg-surface rounded-xl border border-outline p-4 shadow-sm">
            <div className="flex justify-between items-center mb-4">
              <h3 className="text-lg font-semibold text-text-primary">Tareas pendientes</h3>
              <button onClick={() => navigate("/tasks")} className="text-primary text-xs font-medium hover:underline">
                Ver todas
              </button>
            </div>
            {pendingTasks.length === 0 ? (
              <p className="text-sm text-text-secondary text-center py-8">No hay tareas pendientes</p>
            ) : (
              <ul className="flex flex-col gap-2">
                {pendingTasks.slice(0, 5).map((task) => (
                  <li key={task.id} className="flex items-center justify-between p-3 rounded-lg hover:bg-surface-variant border border-transparent hover:border-outline transition-all group">
          ...
          <div className="md:col-span-6 bg-surface rounded-xl border border-outline p-4 shadow-sm">
            <div className="flex justify-between items-center mb-4">
              <h3 className="text-lg font-semibold text-text-primary">Metas de ahorro</h3>
              <button onClick={() => navigate("/savings")} className="text-primary text-xs font-medium hover:underline">
                Ver todas
              </button>
            </div>
            {isLoading ? (
              <div className="text-center py-8 text-sm text-text-secondary">Cargando...</div>
            ) : savings.length === 0 ? (
              <p className="text-sm text-text-secondary text-center py-8">No hay metas de ahorro</p>
            ) : (
                  ...
                  return (
                    <div key={goal.id} className="flex flex-col gap-1.5">
                      <div className="flex justify-between items-end">
                        <div>
                          <p className="text-sm font-semibold">{goal.target_name || goal.name}</p>
                          <p className="text-xs text-text-secondary">
