...
export default function SettingsPage() {
  ...
  const handleAdminDeleteUser = async (targetUser) => {
    if (
      !window.confirm(
        `¿Eliminar a "${targetUser.name}"?\n\nEsta acción no se puede deshacer. Todos sus datos serán eliminados.`,
      )
    )
      return;
    ...
    <div className="max-w-3xl mx-auto space-y-6">
      <div>
        ...
        <p className="text-sm text-text-secondary mt-1">
          Administración de tu cuenta y usuarios
        </p>
      </div>
      ...
      <div className="bg-surface rounded-xl border border-outline p-5 space-y-4">
        ...
        <div className="flex flex-col gap-2">
          <button
            ...
            className="flex items-center gap-3 px-4 py-2.5 rounded-lg hover:bg-surface-variant transition-colors text-sm text-text-primary"
          >
            <span className="material-symbols-outlined text-[20px] text-text-secondary">
              lock
            </span>
            Cambiar contraseña
            <span className="material-symbols-outlined text-[18px] text-text-secondary ml-auto transition-transform">
          ...
          {passwordOpen && (
            <form
              ...
              className="ml-9 pl-4 border-l-2 border-outline space-y-3 pb-2"
            >
              {passwordSuccess && (
              ...
              <div className="space-y-1">
                ...
                <input
                  ...
                  className="w-full px-3 py-2 bg-surface border border-outline rounded-lg text-sm text-text-primary focus:ring-2 focus:ring-primary/20 focus:border-primary outline-none transition-all"
                  required
                />
              </div>
              <div className="space-y-1">
              ...
              <button
                ...
                className="px-4 py-2 bg-primary text-on-primary rounded-lg text-sm font-medium hover:bg-primary-hover disabled:opacity-50 transition-all"
              >
                {passwordSubmitting
                  ? "Actualizando..."
          ...
          <button
            ...
            className="flex items-center gap-3 px-4 py-2.5 rounded-lg hover:bg-error/5 transition-colors text-sm text-error"
          >
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b border-outline bg-surface-variant/50">
                    ...
                    <th className="text-center px-5 py-3 text-xs font-medium text-text-secondary uppercase tracking-wider">
                      Rol
                    </th>
                    <th className="text-right px-5 py-3 text-xs font-medium text-text-secondary uppercase tracking-wider">
                      Acción
                  ...
                  {users.map((u) => (
                    ...
                    >
                      ...
                      <td className="px-5 py-4 text-right text-text-primary font-medium">
                        {u.salary
                          ? `$${Number(u.salary).toLocaleString()}`
                          : "—"}
                      </td>
                        ...
                        <div className="flex items-center justify-end gap-1">
                          ...
                          {u.id !== user?.id && (
                            <button
                              ...
                              className="p-1.5 text-text-secondary hover:text-error transition-colors disabled:opacity-40"
                            >
                              {deletingId === u.id ? (
                                <svg
